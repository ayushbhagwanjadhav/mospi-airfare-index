...
def generate_mospi_index(clean_data_path="data/cleaned_dataset.csv", 
    ...
    # 2. Dynamic Baseline Initialization (UPGRADED TO MEDIAN)
    if not os.path.exists(base_fares_path):
        ...
        base_df = metrics[metrics['Observation_Date'] == first_obs_date][
            ['Origin_Destination', 'Advance_Purchase_Window', 'Median_Fare_INR']
        ].copy()
        base_df.rename(columns={'Median_Fare_INR': 'Base_Fare_INR'}, inplace=True)
    ...
    # 3. Calculate CPI for each observation date (UPGRADED TO MEDIAN)
    def calculate_cpi(row):
        key = (row['Origin_Destination'], row['Advance_Purchase_Window'])
        current_price = row['Median_Fare_INR']
    ...
    metrics['MoSPI_Base_Fare'] = metrics.apply(
        lambda r: base_fares_dict.get((r['Origin_Destination'], r['Advance_Purchase_Window']), r['Median_Fare_INR']), 
        axis=1
    )
    ...
    window_order = {'T+1': 1, 'T+7': 2, 'T+15': 3, 'T+30': 4, 'T+45': 5}
    ...
    print(latest_snapshot[['Origin_Destination', 'Advance_Purchase_Window', 'Median_Fare_INR', 'Airfare_CPI_Index', 'Inflation_Percentage']].to_string(index=False))
